...
# v3.01-v4.3[](#v3-01-v4-3 "Link to this heading")

## Parámetros de sistema y propiedades extendidas[](#parametros-de-sistema-y-propiedades-extendidas "Link to this heading")
...
> [![Instalador de upgrade](_images/QflowDatabaseInstaller.png)](_images/QflowDatabaseInstaller.png)
> 
> Instalador de upgrade[](#id7 "Link to this image")

Al hacer clic en “Next”, se mostrará un resumen de la información ingresada, similar a la siguiente imagen:

> [![Confirmar datos de instalación](_images/QflowDatabaseInstallerSummary.png)](_images/QflowDatabaseInstallerSummary.png)
> 
> Confirmar datos de instalación[](#id8 "Link to this image")
...
En adición, se agregarán los distintos sitios y servicios, teniendo como servidor el ingresado como Web server name, en la tabla RegisteredService. Si se quiere realizar alguna modificación se deberá hacer desde la base de datos. Para más información sobre dicha tabla, vea el manual [Modelo de Base de Datos](13-DatabaseModel.html).

Para corroborar que la importación de la configuración haya sido correcta, podrá hacerlo desde SAM Web, luego de su instalación. Para más información vea el manual [Administración y monitoreo del sistema](19-QflowAdmin.html).
...
## Habilitar configuración de formularios personalizados del sitio WebForms[](#habilitar-configuracion-de-formularios-personalizados-del-sitio-webforms "Link to this heading")
...
## Consideraciones de migración desde Q-flow 3.6 o anterior[](#consideraciones-de-migracion-desde-q-flow-3-6-o-anterior "Link to this heading")

### Validación de campo requerido tipo CheckBox[](#validacion-de-campo-requerido-tipo-checkbox "Link to this heading")
...
### Propiedades de adjuntos[](#propiedades-de-adjuntos "Link to this heading")

El nuevo sitio MVC no implementa las propiedades de los adjuntos de los procesos.

### Sitio WebForms deprecado[](#sitio-webforms-deprecado "Link to this heading")
...
### Sitio Mobile deprecado[](#sitio-mobile-deprecado "Link to this heading")
...
## Consideraciones de migración desde qflow 3.5 o anterior[](#consideraciones-de-migracion-desde-qflow-3-5-o-anterior "Link to this heading")
...
### Actualización de las Web Parts de SharePoint[](#actualizacion-de-las-web-parts-de-sharepoint "Link to this heading")

Las web parts de SharePoint han sido actualizadas para ser distribuidas como un solution package (wsp). Por este motivo la nueva versión de las web parts se soporta solamente en versiones de SharePoint 2010 y posteriores.

Si su organización utiliza la versión anterior de las web parts en SharePoint 2003 o 2007, puede seguir usándolas. Sin embargo, la versión anterior de las web parts no se mantendrá y a futuro podrían surgir cambios en los web services que generen incompatibilidades.

### Cambios en el cuadro de texto enriquecidoVerificación de consistencia de datos ingresados programáticamente[](#cambios-en-el-cuadro-de-texto-enriquecidoverificacion-de-consistencia-de-datos-ingresados-programaticamente "Link to this heading")
...
### Cambios relacionados con localización en tiempo de ejecución[](#cambios-relacionados-con-localizacion-en-tiempo-de-ejecucion "Link to this heading")
...
>     
>     La sentencia anterior tenía un comportamiento diferente en un servidor en español que en otro que se encontraba en inglés. Notar que si se estaba utilizando objetos de tipos numéricos (como int o decimal) o fechas (DateTime) este cambio no afectará en nada y el código seguirá funcionando igual que antes. A modo de ejemplo, la siguiente sentencia no cambia su significado en lo absoluto:
>     
>     Host.GetData("número").Value \= 1.23;
>     
> *   En la API de código personalizado se elimina la propiedad Culture disponible en objetos de tipo User. Dicha propiedad no proporcionaba información útil, ya que los usuarios en Q-flow no tienen asociado un idioma a nivel de sistema.
>     
...
### Cambios en el formato de exportación[](#cambios-en-el-formato-de-exportacion "Link to this heading")
...
## Consideraciones de migración desde qflow 3.4 o anterior[](#consideraciones-de-migracion-desde-qflow-3-4-o-anterior "Link to this heading")

### Requisitos de software de base[](#requisitos-de-software-de-base "Link to this heading")
...
### ASP.NET[](#asp-net "Link to this heading")
...
### Configuración de sistema[](#configuracion-de-sistema "Link to this heading")
...
### Cambios en Web Services[](#cambios-en-web-services "Link to this heading")
...
### Verificación de consistencia de datos ingresados programáticamente[](#verificacion-de-consistencia-de-datos-ingresados-programaticamente "Link to this heading")
...
## Consideraciones de migración desde qflow 3.3 o anterior[](#consideraciones-de-migracion-desde-qflow-3-3-o-anterior "Link to this heading")

### Formularios personalizados[](#id1 "Link to this heading")
...
### Diseño de procesos[](#diseno-de-procesos "Link to this heading")
...
### Exportación e importación[](#exportacion-e-importacion "Link to this heading")
...
## Consideraciones de migración desde qflow 3.2 o anterior[](#consideraciones-de-migracion-desde-qflow-3-2-o-anterior "Link to this heading")

### Formularios personalizados[](#id2 "Link to this heading")
...
### Hojas de estilos (CSS)[](#hojas-de-estilos-css "Link to this heading")
...
## Consideraciones de migración desde qflow 3.1 o anterior[](#consideraciones-de-migracion-desde-qflow-3-1-o-anterior "Link to this heading")

### Formularios personalizados[](#id3 "Link to this heading")
...
### Diseñador de procesos[](#disenador-de-procesos "Link to this heading")
...
## Consideraciones de migración desde qflow 3.05 o anterior[](#consideraciones-de-migracion-desde-qflow-3-05-o-anterior "Link to this heading")

### Cola de mensajes[](#cola-de-mensajes "Link to this heading")

Asegúrese que las colas de mensajes creadas por Q-flow (de novedades y notificaciones) estén vacías. Esto es especialmente importante si usted está migrando a Q-flow 3.1 o posterior, ya que las colas de mensajes dejan de ser utilizadas a partir de dicha versión, por lo que cualquier mensaje pendiente no será procesado por los motores. En caso de que la cola no esté vacía, deshabilite el acceso web de Q-flow y los Web Services, de modo que no se realicen operaciones nuevas sobre el sistema, y espere a que el backend procese todos los mensajes pendientes.
...
### Consideraciones para migración de datos en Base de Personalización[](#consideraciones-para-migracion-de-datos-en-base-de-personalizacion "Link to this heading")
...
### Consideraciones de acceso a datos de aplicación para actualización de Base de Datos[](#consideraciones-de-acceso-a-datos-de-aplicacion-para-actualizacion-de-base-de-datos "Link to this heading")
...
### Compatibilidad de funcionalidades con versiones de Servidor de Base de Datos[](#compatibilidad-de-funcionalidades-con-versiones-de-servidor-de-base-de-datos "Link to this heading")
...
## Consideraciones de migración desde Q-flow 3.03 o anterior[](#consideraciones-de-migracion-desde-q-flow-3-03-o-anterior "Link to this heading")
...
### Uso de Ajax[](#uso-de-ajax "Link to this heading")
...
Solución: si usa el control ScriptManager o ToolScriptManager para trabajar con controles Ajax (por ejemplo UpdatePanel), simplemente debe borrar de su página dicho control. Si a su vez utiliza otras características Ajax, tales como registrar archivos javascript ([\*](#id4).js) o un web service, reemplace el control ScriptManager por el control ScriptManagerProxy.

## Consideraciones de migración desde Q-flow 3.01 o anterior[](#consideraciones-de-migracion-desde-q-flow-3-01-o-anterior "Link to this heading")

### Formularios personalizados[](#id6 "Link to this heading")
...
> 
> *   La rutina de validación javascript se modifica para recibir argumentos necesarios para el framework de validación de ASP.NET. En el caso de ejemplo que estamos viendo la firma sería la siguiente: function validarForm(source, clientside\_arguments)
>     
> *   Dentro de la rutina de validación javascript se utiliza la propiedad booleana clientside\_arguments.IsValid para especificar si la validación fue exitosa o no. Si la validación no es exitosa no se realiza el postback.
>     
