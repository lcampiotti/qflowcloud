...
El propósito de este manual es describir la API de web services de Qflow para apoyar a desarrolladores de aplicaciones que los utilizan.

Este manual contiene solamente información acerca de la interfaz y del comportamiento de los web services.

## Organización de este manual[](#organizacion-de-este-manual "Link to this heading")

Primero encontrará una explicación de cómo invocar cualquier servicio, incluyendo cómo hacer uso de la interfaz Swagger. Luego, Qflow incluye los siguientes web services:
...
Para invocar los servicios expuestos en instalaciones **Cloud**, se debe usar la URL _https://{espacioDeTrabajo}.api.qflowbpm.com_, donde _{espacioDeTrabajo}_ debe ser reemplazado por el nombre de su espacio de trabajo sin espacios.
...
El web service Status simplemente cuenta con un método y es utilizado para verificar el estado de los web services.

#### Métodos de Status[](#metodos-de-status "Link to this heading")

*   **GetStatus():ServiceStatusModel** - método anónimo que devuelve un objeto con el estado de los web services.
...
*   **GetCurrentUserTasksByTemplatesNamesWithDataNames(string\[\] templatesFilter, string\[\] dataToInclude):DataTable** – similar al anterior, pero en lugar de utilizar los identificadores de los templates y datos cuya información se deben traer, se utilizan sus nombres.
...
*   **FinalizeWaitingStepsByName(Guid flowId, string templateStepName):void** – indica que un evento de atrapar señal debe terminar la espera por una acción externa. El evento es especificado por el identificador del proceso al que pertenece y el nombre del paso.
    
*   **FinalizeWaitingStepsById(Guid flowId, Guid templateStepId):void** – indica que un evento de atrapar señal debe terminar la espera por una acción externa. El evento es especificado por el identificador del proceso al que pertenece y el identificador del paso del template en el que se basa.
    
*   **GetFlowWaitingSteps(Guid flowId):FlowStepMessage\[\]** – dado el identificador de un proceso (“flowId”), devuelve una lista de objetos que representan los eventos de atrapar señal de ese proceso que estén en estado de espera.
...
*   **GetDefaultUserProperties():DefaultUserPropertiesMessage** – devuelve un objeto que representa la configuración por defecto que es asignada a los nuevos usuarios
...
*   **GetOrganizationNode(Guid nodeId):OrganizationNodeMessage** – devuelve un objeto que representa el nodo identificado por “nodeId”.
...
*   **GetUsersInGroup(Guid groupId, bool excludeSubNodeMembers):SimpleUserMessage\[\]** – dado el identificador de un grupo, devuelve todos sus usuarios. Si “excludeSubNodeMembers” es “false”, devuelve también los usuarios de todos los nodos miembros de ese grupo. De lo contrario, devuelve solamente los usuarios del grupo cuyo identificador fue indicado.
...
*   **ModifyOrganizationNode(OrganizationNodeMessage node):void** – dado un objeto OrganizationNodeMessage que representa un nodo organizacional, modifica los datos de ese nodo, reemplazando sus valores por los que son proporcionados en el parámetro. Es conveniente invocar el método GetOrganizationNode antes de invocar éste, para obtener un objeto que contenga los datos actualizados del nodo. Después, se modifica el objeto obtenido y se lo utiliza como parámetro para invocar este método.
...
*   **SaveDefaultUserConfiguration(DefaultUserPropertiesRequest inputMessage):void** – modifica la configuración por defecto que es asignada a los nuevos usuarios
...
*   **CheckInTask(Guid flowId, Guid taskId, Guid taskToId):void** – hace que el usuario con cuyas credenciales se está invocando el web service deje una tarea de una cola de trabajo. Para determinar dicha tarea se usan los identificadores del proceso (flowId), la tarea (taskId) y de la parte de esa tarea que es propia del usuario (taskToId). La tarea tiene que haber sido tomada por el usuario antes (por ejemplo, con el método CheckOutTask).
    
*   **CheckOutTask(Guid flowId, Guid taskId, Guid taskToId):void** – hace que el usuario con cuyas credenciales se está invocando el web service tome una tarea de una cola de trabajo. Para determinar dicha tarea se usan los identificadores del proceso (flowId), la tarea (taskId) y de la parte de esa tarea que es propia del usuario (taskToId). La tarea tiene que estar dirigida a una cola de trabajo para la cual el usuario tenga permisos de actuar.
...
*   **CheckInAttachment(Guid flowId, Guid stepId, Guid toId, Guid attachId):void** - Protege el archivo adjunto identificado por “attachId”. Esto es análogo a hacer clic en el botón “Proteger” para ese adjunto en el sitio web viejo. Esta funcionalidad está deprecada.
    
*   **CheckOutAttachment(Guid flowId, Guid stepId, Guid toId, Guid attachId):void** - Desprotege el archivo adjunto identificado por “attachid”. Esto es análogo a hacer clic en el botón “Desproteger” para ese adjunto en el sitio web viejo. Esta funcionalidad está deprecada.
...
*   **UndoCheckOutAttachment(Guid flowId, Guid stepId, Guid toId, Guid attachId):void** - Deshace la desprotección del archivo adjunto identificado por “attachId”. Esto es análogo a hacer clic en el botón “Deshacer desprotección” para ese adjunto en el sitio web viejo. Esta funcionalidad está deprecada.
